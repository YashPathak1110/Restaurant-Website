      ...
      <ul>
        {cartItems.map((item, index) => (
          <li key={index}>
            ...
            <span style={{ fontSize: "40px", color: "white", marginLeft: "7rem" }}>{item.title} - ${item.price} - Quantity: {item.quantity} - Total: ${getTotalPrice(item)}</span><br></br><br></br>
            <button onClick={() => removeFromCart(index)} style={{ marginLeft: "40rem", fontSize: "23px" }} className='btn btn-danger'>Remove</button>
            <button onClick={() => handleIncreaseQuantity(index)} style={{ marginLeft: "5rem", fontSize: "23px" }} className='btn btn-success'>Increase Quantity</button><br></br>
          </li>
        ))}
      </ul>
